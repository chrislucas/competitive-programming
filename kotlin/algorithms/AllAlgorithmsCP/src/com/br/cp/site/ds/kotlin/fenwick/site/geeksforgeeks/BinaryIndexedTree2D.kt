package src.com.br.cp.site.ds.kotlin.fenwick.site.geeksforgeeks
...
operator fun IntMatrix.set(x: Int, y: Int, value: Int) {
    this[x][y] = value
}

operator fun IntMatrix.get(x: Int, y: Int) = this[x][y]

class BinaryIndexedTree2D(private val values: IntMatrix) {

    private val dim = IntPair(values.size, values[0].size)
    ...
    private fun updateTree(pos: IntPair, value: Int) {
        var (px, py) = pos
        val (x, y) = dim
        while (px <= x) {
            while (py <= y) {
    ...
    fun rangeSum(p: IntPair, q: IntPair): Int {
        val (xa, ya) = p
        val (xb, yb) = q
        val a = sum(xb, yb)
        val b = sum(xb, ya - 1)
...
private class TestCase(private val tree: BinaryIndexedTree2D, private val operations: List<Operation>) {
    sealed class Operation
    data class Range(val p: IntPair, val q: IntPair) : Operation()
